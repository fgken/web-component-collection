...
const getAxisYDomain = (from: number, to: number, ref: string, offset: number) => {
  ...
  let [bottom, top]: Array<number> = [refData[0][ref], refData[0][ref]]
  refData.forEach((d) => {
...
export default class TimeSeriesLineChart extends PureComponent<{}, TimeSeriesLineChartState> {
  ...
  zoom() {
    ...
    // yAxis domain
    const [bottom, top] = getAxisYDomain(refAreaLeft as number, refAreaRight as number, 'cost', 1)
    const [bottom2, top2] = getAxisYDomain(refAreaLeft as number, refAreaRight as number, 'impression', 50)
